...
export const make = async (powers, context) => {
  ...
  const nonceLocator = E(powers).nonceLocator();
  ...
  void E(context).addDisposalHook(() => stopped);

  const connect = async (address, context) => {
    const { value: connectionNumber } = connectionNumbers.next();
    ...
    const { reader: bytesReader, writer: bytesWriter, closed: connectionClosed } = await connectPort({
      port,
      host,
      cancelled,
    });
  ...
  return Far('TcpNetstringService', {
    ...
    supports: () => harden(['tcp+netstring+json+captp0']),
    connect,
  });
};
